...
'PriorityBlockingQueue':[
    '带优先级的无界阻塞队列',
    '使用二叉树堆维护元素优先级,使用数组作为元素存储的数据结构，数组可扩容',
    '出队时始终保证出队元素是堆树的根节点(优先级最高或者最低的元素)',
    {'结构':[
        '数组queue:存放队列元素,默认容量11,可扩容',
        'size:存放队列元素个数',
        'allocationSpinLock:自旋锁，使用CAS操作保证同时只有一个线程扩容队列，状态为0或1',
        'comparator比较器:比较元素大小,默认使用对象compareTo方法,可自定义',
        'lock独占锁:控制同时只有一个线程可进行入队、出队操作',
        'notEmpty条件变量:实现take方法阻塞模式',
        '无notFull条件变量:无界队列,put操作非阻塞',
    ]},
    {'方法':[
        'offer:在队列中插入一个元素，由于是无界队列，所以一直返回true',
        'poll:获取队列内部堆树的根节点元素，如果队列为空，则返回null',
        'put:内部调用的是offer操作，由于是无界队列，所以不需要阻塞',
        'take:获取队列内部堆树的根节点元素，如果队列为空则阻塞'
    ]},
...
'DelayQueue':[
    ...
    {'结构':[
        ...
        'available条件变量:与lock锁对应的，实现线程间同步',
