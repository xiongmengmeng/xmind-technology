...
'IoC容器':[
    ...
    {'BeanFactory':[
       '一个顶级容器接口,spring是所有的IoC容器都需要实现它',
       {'方法':[
           '多个getBean方法:按类型（by type）/名称（byname）获取Bean',
           'isSingleton:Bean是否为单例,默认Bean都是单例的，即使用getBean方法返回的都是同一个对象',
    ...
    {'AnnotationConfigApplicationContext':[
       ...
       {'参数':[
           ...
           'ClassPathBeanDefinitionScanner：一个扫描指定类路径中注解Bean定义的扫描器，它初始化时，会初始化一些需要被扫描的注解'
       ]}
    ]},
    {'AnnotatedBeanDefinitionReader初始化':[
       ...
       '6.EventListenerMethodProcessor:r提供@ EventListener  的支持'
    ]},
