...
'ThreadLocalRandom':[
    ...
    {'种子':[
        '存在调用线程的threadLocalRandomSeed变量',
        '根据线程内维护的种子变量计算新种子，避免了竞争'
    ]},
    {'current()':[
        'threadLocalRandomSeed变量--初始化种子',
...
'AtomicLong':[
    {'变量':[
        'Unsafe.getUnsafe()方法获取Unsafe类的实例',
        'valueOffset:变量value的偏移量',
        'value:被声明为volatile的，在多线程下保证了内存的可见性'
    ]},
    {'getAndIncrement':[
        'boolean compareAndSet(long expect, long update)----JDK 7',
        'unsafe.getAndAddLong----JDK 8',
