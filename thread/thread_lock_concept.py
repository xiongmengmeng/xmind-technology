...
'伪共享':[
    '产生原因：因为多个变量被放入了一个缓存行中，并且多个线程同时去写入缓存行中不同的变量',
    '缓存与内存交换数据的单位就是缓存行，当CPU要访问的变量没有在缓存中找到时，根据程序运行的局部性原理，会把该变量所在内存中大小为缓存行的内存放入缓存行'
],
'偏向锁/轻量级锁/重量级锁':[
    '通过对象监视器在对象头中的字段来表明的',
    ...
    {'轻量级锁':[
        ...
        {'过程':[
            ...
            '拷贝成功，虚拟机使用CAS尝试将对象的Mark Word更新为指向Lock Record的指针，并将Lock Record里的owner指针指向对象的Mark Word',
            '如更新动作成功，这个线程就拥有了该对象的锁，对象Mark Word的锁标志位设置为“00”，表示此对象处于轻量级锁定状态',
            '如轻量级锁更新失败，虚拟机会检查对象的Mark Word是否指向当前线程的栈帧，如果是说明当前线程已拥有这个对象的锁，可以直接进入同步块继续执行，否则说明多个线程竞争锁'
        ]},
        {'自旋':[
            '好处:减少线程上下文切换的消耗',
            '缺点:循环会消耗CPU'
...
'自旋锁 VS 适应性自旋锁 ':[
    {'自旋锁':[
        ...
        '如锁被占用时间很长，自旋线程只会浪费处理器资源。所以自旋等待时间要有一定限度，如自旋超过限定次数（默认是10次，可以使用-XX:PreBlockSpin来更改）没有成功获得锁，就应当挂起线程',
        'do-while循环就是一个自旋操作',
        'JDK 6中变为默认开启',
    ...
    {'适应性自旋锁':[
        ...
        '如在同一个锁对象上，自旋等待刚刚成功获得过锁，并且持有锁的线程正在运行中，虚拟机会认为这次自旋也很有可能成功，进而允许自旋等待更长时间',
...
'可重入锁 VS 非可重入锁':[
    ...
    {'释放锁':[
        '可重入锁:先获取当前status的值，在当前线程是持有锁的线程的前提下,如status-1 == 0，表示当前线程所有重复获取锁的操作都已经执行完毕，该线程会真正释放锁',
