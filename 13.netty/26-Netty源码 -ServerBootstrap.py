...
'ServerBootstrap':[
    ...
    {'doBind(localAddress)':[
        {'initAndRegister()':[
            '反射创建NioServerSocketChannel及其相关的NIO对象,pipeline(初始head节点和tail节点),unsafe',
            ...
            {'init(channel)':[
                '初始化NioServerSocketChannel对象，设置参数，添加处理器',
                '将自定义的处理器，封装成AbstractChannelHandlerContext对象添加到ChannelPipeline',
                '将ServerBootstrapAcceptor，封装成AbstractChannelHandlerContext对象添加到ChannelPipeline'
            ]},
