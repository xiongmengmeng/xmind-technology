...
'exec()函数':[
    '一组函数的统称, 包括execl()、execlp()、execv()、execle()、execve()、execvp()',
    '作用：装载一个新的程序（可执行映像）覆盖当前进程内存空间中的映像，从而执行不同的任务',
    '注：exec系列函数在执行时会直接替换掉当前进程的地址空间(清空原有的数据)'
],
'Copy On Write':[
    {'技术实现原理':[
        'fork()后，父进程中所有内存页的权限被设为read-only，然后子进程的地址空间指向父进程',
        '当父子进程都只读内存时，相安无事',
        '当其中某个进程写内存时，CPU硬件检测到内存页是read-only，触发页异常中断（page-fault），陷入一个中断例程',
