...
'每种数据类型内部编码规则及优化方式':[
    ...
    {'散列类型':[
        ...
        {'内部编码方式':[
            {'REDIS_ENCODING_ZIPLIST':[
                '字段个数<hash-max-ziplist-entries&&字段名和值长度<hash-max-ziplist-value参数值',
                '紧凑的编码格式，它牺牲了部分读取性能以换取极高的空间利用率，适合在元素较少时使用',
                {'结构':[
                    'zlbytes:uint32_t类型，表示整个结构占用的空间',
                    'zltail：uint32_t类型，表示到最后一个元素的偏移，程序可以直接定位到尾部元素',
                    'zllen：uint16_t类型，存储的是元素的数量',
                    {'元素(4部分组成)':[
                        '存储前一个元素的大小(可实现倒序查找)',
                        '元素编码类型',
                        '元素大小',
    ...
    {'有序集合类型':[
        ...
        {'REDIS_ENCODING_SKIPLIST':[
            ...
            '散列表用来存储元素值与元素分数的映射关系以实现O(1)时间复杂度的ZSCORE等命令。',
            '跳跃列表用来存储元素的分数及其到元素值的映射以实现排序的功能',
            {'Redis对跳跃列表的实现':[
                '允许跳跃列表中的元素（即分数）相同',
