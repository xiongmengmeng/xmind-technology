...
s2=w.createSheet()
s2.setTitle("Netty:Handler处理器")
r2=s2.getRootTopic()
r2.setTitle("Netty:Handler处理器")
...
'ChannelHandler':[
    '一个接囗，处理IO事件并将其转发到其ChannelPipeline(业务处理链)中的下一个处理程序',
    {'实现类':[
        {'ChannelInboundHandler':[
            '处理入站IO事件',
            '子类ChannelInboundHandlerAdapter'
        ]},
        {'ChannelOutboundHandler':[
            '处理出站IO事件',
            '子类ChannelOutboundHandlerAdapter'
        ]},
    ]}
],
'ChannelInboundHandler':[
    '通道入站处理器，默认实现为ChannelInboundHandlerAdapter(通道入站处理适配器)',
...
'ChannelOutboundHandler':[
    '通道出站处理器，默认实现为ChanneloutBoundHandlerAdapter(通道出站处理适配器)',
    {'主要操作':[
        {'bind()':[
...
'ChannelInitializer':[
    '通道初始化处理器，属于入站处理器的类型,向流水线中装配业务处理器',
    {'initChannel()方法':[
