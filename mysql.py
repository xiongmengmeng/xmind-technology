...
'30.查这么多数据，会不会把数据库内存打爆':[
    {'全表扫描对server层的影响':[
        '边读边发:查询结果分段发给客户端的，扫描全表，返回大量数据，并不会把内存打爆',
        {'取发数据流程':[
            '1.获取一行，写到net_buffer(大小由net_buffer_length定义，默认16k)',
            '2.重复获取行，直到net_buffer写满，调用网络接口发出去',
            '3.发送成功，清空net_buffe，继续取下一行，并写入net_buffer',
            '4.如发送函数返回EAGAIN或WSAEWOULDBLOCK:本地网络栈（socket send buffer）满，等待,到重新可写，再发送'
        ]},
    ...
    {'对InnoDB的影响':[
        ...
        'Buffer Pool大小由参数innodb_buffer_pool_size确定，建议设置成可用物理内存的 60%~80%',
        '淘汰一个旧数据页:用优化后的LRU算法,链表实现,young区域:old区域=5:3',
        {'改进的LRU算法执行流程':[
            '1.访问数据页 P3(在 young 区)，将其移到链表头部',
            '2.访问一个新的不在当前链表的数据页，淘汰掉数据页 Pm，新插入的数据页 Px放在LRU_old处',
...
'31.join':[
    ...
    '两个表按照各自的条件过滤，过滤完成后，计算参与 join 的各个字段的总数据量，数据量小的表，应作为驱动表',
    {'优化':[
        'MRR算法:顺序读盘，查询语句在索引a上做一个范围查询，得到足够多主键id,排序以后，再去主键索引查数据，',
        'BKA算法:对NLJ算法的优化,暂存驱动表的数据到join_buffer，依赖于 MRR',
    ]}
],
'32.临时表':[
...
'33.内部临时表':[
    '存放语句执行过程中的中间数据:sort buffer、内存临时表和 join buffer',
    '排序用sort buffer，join语句用join buffer',
    {'union 执行流程':[
        'union 要去重，需用到内部临时表',
    ...
    {'group by':[
        ...
        '优化2：索引:确保输入数据是有序的',
        '优化3：直接排序,语句中加入SQL_BIG_RESULT提示，告诉优化器：语句涉及的数据量大，请直接用磁盘临时表',
        'MySQL 的优化器觉得磁盘临时表是 B+ 树存储，存储效率不如数组来得高，直接用数组来存'
    ]},
    {'MySQL什么时候使用内部临时表':[
        '语句执行过程可一边读数据，一边得到结果，是不需要额外内存的，否则需要额外内存来保存中间结果',
        'join_buffer 是无序数组，sort_buffer 是有序数组，临时表是二维表结构',
        '执行逻辑用到二维表特性，优先考虑使用临时表,比union 用到唯一索引约束，group by 用到另外一个字段来存累积计数'
    ]}
],
'34.Memory引擎':[
    'Memory 引擎的数据和索引是分开的',
