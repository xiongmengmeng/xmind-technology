...
'3.主从复制：多副本':[
    ...
    {'优点':[
        '缩短不可用时间：master发生宕机，可手动把slave提升为master继续提供服务',
    ...
    {'slave node做复制时':[
        '不阻塞master的正常工作和自己的查询操作（用旧数据集提供服务）',
...
'4.哨兵：故障自动切换':[
    ...
    {'优点':[
        '实现redis集群高可用，本身也是分布式的（至少需3个实例，保证自己的健壮性）',
    ]},
    {'缺陷':[
        '写操作无法负载均衡；存储能力受到单机的限制',
        '哨兵+redis主从的部署架构，不保证数据零丢失'
    ]},
    {'与zk的区别':[
        {'相同':[
            '故障转移都需要主节点操作'
...
'5.分片集群：横向扩展':[
    ...
    '部署多个实例，按照一定规则将实例组织起来，把它们当成一个整体，对外提供服务',
    {'分片集群两大类(根据路由规则所在位置)':[
        '客户端分片:Redis Cluster',
        '服务端分片:代理分片Twemproxy、Codis'
    ]}
]
