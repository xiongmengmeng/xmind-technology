...
'CAS':[
    ...
    {'效果':[
        '多个线程尝试使用CAS同时更新同一个变量，只有一个线程能更新变量的值',
        '其他线程都失败，失败线程不会被挂起，而是被告知这次竞争中失败，可再次尝试'
    ]},
    {'三个操作数':[
        '需要读写的内存位置（V）',
        '进行比较的预期原值（A）',
...
'AQS':[
    ...
    {'重要元素':[
        {'state':[
            ...
            '对于ReentrantLock:state表示当前线程获取锁的可重入次数',
            '对于ReentrantReadWriteLock:state高16位表示读状态(获取该读锁的次数)，低16位表示写状态',
            '对于semaphore:state表示当前可用信号的个数',
        ...
        {'内部类ConditionObject':[
            ...
            {'await()':[
                ...
                '2.之后当前线程会释放获取的锁（会操作锁对应的state变量的值），并被阻塞挂起'
            ]},
            {'signal()':[
                '1.把条件队列里面队头的一个线程节点从条件队列里移除并放入AQS的阻塞队列里面',
                '2.然后激活这个线程'
            ]},
    ...
    {'核心方法':[
          {'acquire（int arg）':[
            '1.先使用tryAcquire方法尝试获取资源，具体是设置状态变量state的值，成功则直接返回',
            '2.失败则将当前线程封装为类型为Node.EXCLUSIVE的Node节点后插入到AQS阻塞队列的尾部',
        ...
        {'release（int arg）':[
            '1.尝试使用tryRelease操作释放资源，这里是设置状态变量state的值',
            '2.调用LockSupport.unpark（thread）方法激活AQS队列里面被阻塞的一个线程（thread）',
