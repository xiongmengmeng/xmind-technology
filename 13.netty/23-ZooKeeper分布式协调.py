...
'分布式命名服务':[
    ...
    {'ID生成器方案':[
        {'Java的UUID':[
            ...
            '缺点是过长，16字节共128位，通常以36字节长的字符串来表示',
            '同时没有排序，无法保证趋势递增，因此用于数据库索引字段的效率就很低，添加记录存储入库时性能差'
        ]},
        {'分布式缓存Redis生成ID':[
        ...
        {'MongoDb的ObjectId':[
            'MongoDB是一个分布式的非结构化NoSQL数据库',
            '每插入一条记录会自动生成全局唯一的一个“_id”字段值(一个12字节的字符串)，可作为分布式系统中全局唯一的ID'
        ]}
    ]}
],
'分布式锁':[
    ...
    {'ZooKeeper分布式锁':[
        ...
        {'基于ZooKeeper实现一下分布式锁':[
            ...
            '如果某个客户端需要占用锁，则在“/test/lock”下创建一个临时有序的子节点',
            '3.客户端创建子节点后,判断自己创建的子节点是否为当前子节点列表中序号最小的子节点',
            '如果是，则认为加锁成功；如果不是，则监听前一个ZNode子节点的变更消息，等待前一个节点释放锁',
            '4.一旦队列中后面的节点获得前一个子节点的变更通知，进行判断自己是否为当前子节点列表中序号最小的子节点',
            '如果是，则认为加锁成功；如果不是，则持续监听，一直到获得锁',
            '5.获取锁后，开始处理业务流程。在完成业务流程后，删除自己对应的子节点',
            '完成释放锁的工作，以便后面的节点能捕获到节点的变更通知，获得分布式锁'
        ]},
